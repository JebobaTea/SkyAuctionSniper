...
class Snipe(commands.Cog):
    ...
    async def l(self):
        ...
        if auc:
            for auction in auc:
                if auc[auction][1] > 500000 and auc[auction][0]/auc[auction][1] < 1/10 and auc[auction][1] != float("inf") and auc[auction][1] < 100000000 and auc[auction][5] > 10:
                    results.append([auc[auction][2], f"{auc[auction][0]:,}", f"{auc[auction][1]:,}", f"{auc[auction][1]-auc[auction][0]:,}", auction])
            
            for result in results:
                embed = discord.Embed(color = discord.Color.orange(), title = result[4])
                embed.add_field(name = result[0], value = "Being sold for " + str(result[1]) + " at a gap of " + str(result[3]) + " \n Second cheapest: " + str(result[2]), inline = False)
